...
public class CodeBuffer {
    ...
    public void setByteAt(int pos, int x) {
        if (Assert.ASSERTS_ENABLED) {
            Assert.that((pos >= 0) && (pos < count), "index out of bounds");
            Assert.that((address == 0), "code emitted after relocation");
        }
        code[pos] = (byte) x;
    }
    ...
    public void compactCode(int at, int numbytes) {
        if (Assert.ASSERTS_ENABLED) {
            Assert.that(pos - numbytes >= 0, "cannot compact more bytes than available");
        }
        count -= numbytes;
        for (int i = 0; i < count; i++) {
            code[at + i] = code[at + i + numbytes];
        }
        pos -= numbytes;
    }
}
