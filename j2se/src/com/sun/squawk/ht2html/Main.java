...
public class Main {
    ...
    private void run() {
        ...
        String line;
        while ((line = in.readLine()) != null && !line.startsWith("*HEAP*:initialize:")) {
            line = in.readLine();
        }
            ...
            if (line.startsWith("*HEAP*:")) {
                ...
                if (tag.equals("start")) {
                    ...
                    if (heap != null) {
                        // must be an object
                        try {
                            long objectStart = Long.parseLong(tag);
    ...
    private Heap parseStart(String parameters, InputFile in) {
        ...
        try {
            long branchCount = Long.parseLong(st.nextToken());
            long freeMemory = Long.parseLong(st.nextToken());
            long totalMemory = Long.parseLong(st.nextToken());
    ...
    private PrintStream openOutputFile(String name) {
        ...
        try {
            return new PrintStream(new BufferedOutputStream(new FileOutputStream(path), 20000000));
...
class HeapsIndexInstantiator implements Template.Instantiator {
    ...
    private void anchor(PrintStream out, String base, int id) {
        out.println("[<a href=\"" + base + id + ".html\" onClick=\"select('h" + id + "')\" target=\"heapFrame\">" + base + "</a>]");
    }
    ...
    private void doContents(PrintStream out) {
        ...
        while (objects.hasMoreElements()) {
            ...
            while (data.hasMoreElements()) {
                ...
                long word = Long.parseLong((String)data.nextElement());
