...
public class ConstantPool {
     ...
     */
    protected final ClassFileReader cfr;

    /**
     * The class defined by the class file containing this constant pool.
     */
    protected final Klass definedClass;
     ...
     */
    protected void verifyEntry(int index, int tag) {
        if (index < 1 || index >= entries.length) {
            //throw cfr.formatError("constant pool index out of range");
            throw cfr.formatError("constant pool index out of range" + index + " vs " + entries.length);
        }
        if (tags[index] != tag) {
    ...
    public Klass verifyFieldType(String sig) {
        ...
        return Translator.getClass(sig, true);
    }
     ...
     * @throws  ClassFormatError if the signature is invalid
     */
    public MethodSignature verifyMethodType(String sig, boolean specialMethod) {
        ...
        if (signature == null) {
            ...
            if (length > 0 && sig.charAt(0) == '(') {
                ...
                while (offset < length) {
                    ...
                    parameterTypes.addElement(Translator.getClass(
                        sig.substring(offset, nextOffset), true));
                    offset = nextOffset;
                }
                ...
                if (offset < length && (sig.charAt(offset) == ')')) {
                    ...
                    if (specialMethod) {
                        ...
                        if (skipOverFieldType(sig, offset, true) == length) {
                            returnType = Translator.getClass(sig.substring(offset, length), true);
                        }
                    }
                }
            }
        ...
        if (signature == MethodSignature.INVALID) {
        ...
        if (valid) {
            if ((classModifiers & Modifier.INTERFACE) == 0) {
                ...
                if ((modifiers & Modifier.ABSTRACT) != 0) {
                    if ((modifiers & (Modifier.FINAL | Modifier.NATIVE | Modifier.SYNCHRONIZED | Modifier.PRIVATE | Modifier.STATIC)) != 0) {
                        valid = false;
                    }
                }
            } else {
...
/*end[FLOATS]*/
    ...
    public Klass getKlass(int index) {
        ...
        if (entries[index] instanceof String) {
            ...
            if (name.charAt(0) == '[') {
                 ...
                 */
                klass = Translator.getClass(name, true);
            } else {
                /*
                 ...
                 */
                name = name.replace('/', '.');
                klass = Translator.getClass(name, false);
            }
            entries[index] = klass;
        }
    ...
    public Klass getResolvedClass(int index, Context context) {
        ...
        if (klass.getState() == Klass.State.ERROR) {
            Translator.throwNoClassDefFoundError(context.prefix(klass.getName()));
        }
        Translator.instance().load(klass);
        if (!klass.isAccessibleFrom(definedClass)) {
            Translator.throwIllegalAccessError(context.prefix(klass.toString()));
        }
        return klass;
    }
        ...
        if (entries[index] instanceof FieldOrMethod) {
            ...
            if (field == null) {
                ...
                if (field != null) {
                     ...
                     */
                    Translator.throwIncompatibleClassChangeError(context.prefix(field.toString()));
                } else {
                    /*
                     * NoSuchFieldError
                     */
                    String message = nt.sig + ' ' + declaringClass.getName() + '.' + nt.name;
                    Translator.throwNoSuchFieldError(context.prefix(message));
                }
            }
            ...
            if (!field.isAccessibleFrom(definedClass)) {
                Translator.throwIllegalAccessError(context.prefix(field.toString()));
            }
        } else {
     ...
     * @param   isInterfaceMethod true if resolving a method in an interface
     * @param   isStatic          true if resolving a static field
     * @param   context            the context of the request
     * @return  the method reference at <code>index</code>
     */
    public Method getResolvedMethod(int index, boolean isStatic, boolean isInterfaceMethod, Context context) {
        verifyEntry(index, (isInterfaceMethod ? CONSTANT_InterfaceMethodref : CONSTANT_Methodref));
        /*
        ...
        if (entries[index] instanceof FieldOrMethod) {
            ...
            boolean isSpecialMethod = !isInterfaceMethod && (name.endsWith("init>"));
            MethodSignature sig = verifyMethodType(nt.sig, isSpecialMethod);
            Klass returnType = sig.returnType;

            if (!isInterfaceMethod) {
                // Catch any direct calls of _SQUAWK_INTERNAL_init
                Assert.that(!name.equals("_SQUAWK_INTERNAL_init"), "there should not be any calls to methods named _SQUAWK_INTERNAL_init");
                if (name.equals("<init>")) {
                    /*
            ...
            if (method == null) {
                ...
                if (method != null) {
                     ...
                     * IncompatibleClassChangeError
                     */
                    Translator.throwIncompatibleClassChangeError(context.prefix(method.toString()));
                } else {
                    /*
                     * NoSuchMethodError
                     */
                    String message = declaringClass.getName() + '.' + nt.name + nt.sig;
//declaringClass.lookupMethod(name, sig.parameterTypes, returnType, definedClass, isStatic);
                    Translator.throwNoSuchMethodError(context.prefix(message));
                }
            }

            if (isInterfaceMethod != method.getDefiningClass().isInterface()) {
                String message = "interface method '" + name + "' found in " + declaringClass;
                Translator.throwIncompatibleClassChangeError(context.prefix(message));
            }

            /*
             * Ensure the class that defined the field and its type are loaded
             */
            Translator.instance().load(returnType);
            for (int i = 0; i != sig.parameterTypes.length; ++i) {
                Translator.instance().load(sig.parameterTypes[i]);
            }
             ...
             */
             if (!method.isAccessibleFrom(definedClass)) {
                 Translator.throwIllegalAccessError(context.prefix(method.toString()));
             }
        } else {
     ...
     */
    public ConstantPool(ClassFileReader cfr, Klass definedClass) {
            ...
            try {
                switch (tags[i]) {
                    ...
                    case CONSTANT_NameAndType: {
                        ...
                        entries[i] = new NameAndType(getUtf8(nameIndex), (String)entries[descriptorIndex]);
                        break;
                    }
                    case CONSTANT_Fieldref:
                    case CONSTANT_Methodref:
