...
public class StackMap {
     ...
     */
    public static IntHashtable loadStackMap(CodeParser codeParser, ClassFileReader cfr, ConstantPool constantPool) {
        /*
        ...
        if (nmaps == 0) {
            ...
            IntHashtable table = new IntHashtable(nmaps);
            int lastAddress = -1;
            ...
            for (int i = 0 ; i < nmaps ; i++) {
                 ...
                 */
                Klass[] locals = loadStackMapList(codeParser, cfr, constantPool);
                int size = 0;
                for (int j = 0; j != locals.length; j++) {
                    if (locals[j].isDoubleWord()) {
                        size++;
                    }
                    size++;
                }
                ...
                /*
                 * Rebuild the locals array if its real size differs from its logical size
                 */
                if (size != locals.length) {
                    Klass[] physicalLocals = new Klass[size];
                    int javacIndex = 0;
                    for (int j = 0; j < locals.length; j++) {
                        Klass type = locals[j];
                        physicalLocals[javacIndex++] = type;
                        if (type.isDoubleWord()) {
                            physicalLocals[javacIndex++] = Klass.getSecondWordType(type);
                        }
                    }
                    Assert.that(javacIndex == physicalLocals.length);
                    locals = physicalLocals;
                }

                /*
                 * Load the list of types on the operand stack
                 */
                Klass[] stack = loadStackMapList(codeParser, cfr, constantPool);
                 ...
                 * Trace.
                 */
                if (Klass.DEBUG && Tracer.isTracing("maps", codeParser.getMethod().toString())) {
     ...
     */
     private static Klass[] loadStackMapList(CodeParser codeParser, ClassFileReader cfr, ConstantPool constantPool) {
