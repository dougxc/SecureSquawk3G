...
//Assert.that(findInstruction(tail));
    ...
    /**
     * Gets the exception handler corresponding to a given handler entry point.
     *
     * @param   katch  the handler entry point
     * @return  the exception handler corresponding to <code>katch</code> which
     *                 must exist
     */
    public IRExceptionHandler getExceptionHandler(Catch katch) {
        Assert.that(exceptionHandlers != null);
        for (Enumeration e = exceptionHandlers.elements(); e.hasMoreElements();) {
            IRExceptionHandler irHandler = (IRExceptionHandler)e.nextElement();
            if (irHandler.getCatch() == katch) {
                return irHandler;
            }
        }
...
/*if[TYPEMAP]*/
        byte[] typeMap = emitter.getTypeMap();
/*end[TYPEMAP]*/
...
/*if[J2ME.DEBUG]*/
        /*
         * Verify the body and return it.
         */
        if (Assert.ASSERTS_ENABLED) {
            new Verifier().verify(body);
        }
/*end[J2ME.DEBUG]*/
