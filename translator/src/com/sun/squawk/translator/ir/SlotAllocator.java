...
public final class SlotAllocator {
     ...
     */
    private int doSlotClearingAnalysis(Vector localRefs) {
        int cleared = 0;

        if (Klass.DEBUG && Tracer.isTracing("slotclearingdetail", method.toString())) {
            Tracer.traceln("");
...
/*if[FULL_SLOT_CLEARING_ANALYSIS]*/
     ...
     */
    private boolean traverse(Local local, Instruction instruction, Target basicBlock, Vector traversed) {
...
/*end[FULL_SLOT_CLEARING_ANALYSIS]*/
    ...
    public Klass[] transform() {
         ...
         */
        int index = 1;
        Vector sortedSlots = null;
        if (clearedSlotCounter > 0) {
            sortedSlots = new Vector(clearedSlotCounter);
        ...
        for (Enumeration e = slots.elements(); e.hasMoreElements();) {
            ...
            if (!Klass.SQUAWK_64 && k.isDoubleWord()) {
                if (k.getClassID() == CID.LONG) {
                    localTypes[index++] = Klass.LONG2;
                } else {
                    Assert.that(k.getClassID() == CID.DOUBLE);
    ...
    private int setSlotIndex(Slot slot, int index) {
        if (!Klass.SQUAWK_64 && slot.getType().isDoubleWord()) {
            if (MethodBody.LOCAL_LONG_ORDER_NORMAL) {
