...
/**
 * This class defines the switch table used by the Squawk interpreter.
    ...
    protected void do_switch() {

        {
            ...
            bind(OPC.CONST_BYTE);                                       iparmNone();
                                                  pre(FLOW_CHANGE);     do_const_byte();                   post();
            bind(OPC.CONST_SHORT);                                      iparmNone();
                                                  pre(FLOW_CHANGE);     do_const_short();                  post();
            bind(OPC.CONST_CHAR);                                       iparmNone();
                                                  pre(FLOW_CHANGE);     do_const_char();                   post();
            bind(OPC.CONST_INT);                                        iparmNone();
                                                  pre(FLOW_CHANGE);     do_const_int();                    post();
            bind(OPC.CONST_LONG);                                       iparmNone();
                                                  pre(FLOW_CHANGE);     do_const_long();                   post();
            ...
            bind(OPC.THIS_GETFIELD_I);                                  iparmUByte();
            bind(OPC.THIS_GETFIELD_I_WIDE);       pre(FLOW_NEXT);       do_this_getfield(INT);             post();
            bind(OPC.THIS_GETFIELD_B);                                  iparmUByte();
            bind(OPC.THIS_GETFIELD_B_WIDE);       pre(FLOW_NEXT);       do_this_getfield(BYTE);            post();
            bind(OPC.THIS_GETFIELD_S);                                  iparmUByte();
            bind(OPC.THIS_GETFIELD_S_WIDE);       pre(FLOW_NEXT);       do_this_getfield(SHORT);           post();
            bind(OPC.THIS_GETFIELD_C);                                  iparmUByte();
            bind(OPC.THIS_GETFIELD_C_WIDE);       pre(FLOW_NEXT);       do_this_getfield(USHORT);          post();
            bind(OPC.THIS_GETFIELD_O);                                  iparmUByte();
            bind(OPC.THIS_GETFIELD_O_WIDE);       pre(FLOW_NEXT);       do_this_getfield(OOP);             post();
            bind(OPC.THIS_GETFIELD_L);                                  iparmUByte();
            bind(OPC.THIS_GETFIELD_L_WIDE);       pre(FLOW_NEXT);       do_this_getfield(LONG);            post();
            ...
            bind(OPC.THIS_PUTFIELD_I);                                  iparmUByte();
            bind(OPC.THIS_PUTFIELD_I_WIDE);       pre(FLOW_NEXT);       do_this_putfield(INT);             post();
            bind(OPC.THIS_PUTFIELD_B);                                  iparmUByte();
            bind(OPC.THIS_PUTFIELD_B_WIDE);       pre(FLOW_NEXT);       do_this_putfield(BYTE);            post();
            bind(OPC.THIS_PUTFIELD_S);                                  iparmUByte();
            bind(OPC.THIS_PUTFIELD_S_WIDE);       pre(FLOW_NEXT);       do_this_putfield(SHORT);           post();
            bind(OPC.THIS_PUTFIELD_O);                                  iparmUByte();
            bind(OPC.THIS_PUTFIELD_O_WIDE);       pre(FLOW_NEXT);       do_this_putfield(OOP);             post();
            bind(OPC.THIS_PUTFIELD_L);                                  iparmUByte();
            bind(OPC.THIS_PUTFIELD_L_WIDE);       pre(FLOW_NEXT);       do_this_putfield(LONG);            post();
            ...
            bind(OPC.RES_0);                                            iparmNone();
                                                  pre(FLOW_CHANGE);     do_res(0);                         post();
/*if[FLOATS]*/
            bind(OPC.IF_EQ_F);                                          iparmByte();
            bind(OPC.IF_EQ_F_WIDE);               pre(FLOW_CHANGE);     do_if(1, EQ, FLOAT);               post();
            bind(OPC.IF_NE_F);                                          iparmByte();
            bind(OPC.IF_NE_F_WIDE);               pre(FLOW_CHANGE);     do_if(1, NE, FLOAT);               post();
            bind(OPC.IF_LT_F);                                          iparmByte();
            bind(OPC.IF_LT_F_WIDE);               pre(FLOW_CHANGE);     do_if(1, LT, FLOAT);               post();
            bind(OPC.IF_LE_F);                                          iparmByte();
            bind(OPC.IF_LE_F_WIDE);               pre(FLOW_CHANGE);     do_if(1, LE, FLOAT);               post();
            bind(OPC.IF_GT_F);                                          iparmByte();
            bind(OPC.IF_GT_F_WIDE);               pre(FLOW_CHANGE);     do_if(1, GT, FLOAT);               post();
            bind(OPC.IF_GE_F);                                          iparmByte();
            bind(OPC.IF_GE_F_WIDE);               pre(FLOW_CHANGE);     do_if(1, GE, FLOAT);               post();
            bind(OPC.IF_CMPEQ_F);                                       iparmByte();
            bind(OPC.IF_CMPEQ_F_WIDE);            pre(FLOW_CHANGE);     do_if(2, EQ, FLOAT);               post();
            bind(OPC.IF_CMPNE_F);                                       iparmByte();
            bind(OPC.IF_CMPNE_F_WIDE);            pre(FLOW_CHANGE);     do_if(2, NE, FLOAT);               post();
            bind(OPC.IF_CMPLT_F);                                       iparmByte();
            bind(OPC.IF_CMPLT_F_WIDE);            pre(FLOW_CHANGE);     do_if(2, LT, FLOAT);               post();
            bind(OPC.IF_CMPLE_F);                                       iparmByte();
            bind(OPC.IF_CMPLE_F_WIDE);            pre(FLOW_CHANGE);     do_if(2, LE, FLOAT);               post();
            bind(OPC.IF_CMPGT_F);                                       iparmByte();
            bind(OPC.IF_CMPGT_F_WIDE);            pre(FLOW_CHANGE);     do_if(2, GT, FLOAT);               post();
            bind(OPC.IF_CMPGE_F);                                       iparmByte();
            bind(OPC.IF_CMPGE_F_WIDE);            pre(FLOW_CHANGE);     do_if(2, GE, FLOAT);               post();
            bind(OPC.IF_EQ_D);                                          iparmByte();
            bind(OPC.IF_EQ_D_WIDE);               pre(FLOW_CHANGE);     do_if(1, EQ, DOUBLE);              post();
            bind(OPC.IF_NE_D);                                          iparmByte();
            bind(OPC.IF_NE_D_WIDE);               pre(FLOW_CHANGE);     do_if(1, NE, DOUBLE);              post();
            bind(OPC.IF_LT_D);                                          iparmByte();
            bind(OPC.IF_LT_D_WIDE);               pre(FLOW_CHANGE);     do_if(1, LT, DOUBLE);              post();
            bind(OPC.IF_LE_D);                                          iparmByte();
            bind(OPC.IF_LE_D_WIDE);               pre(FLOW_CHANGE);     do_if(1, LE, DOUBLE);              post();
            bind(OPC.IF_GT_D);                                          iparmByte();
            bind(OPC.IF_GT_D_WIDE);               pre(FLOW_CHANGE);     do_if(1, GT, DOUBLE);              post();
            bind(OPC.IF_GE_D);                                          iparmByte();
            bind(OPC.IF_GE_D_WIDE);               pre(FLOW_CHANGE);     do_if(1, GE, DOUBLE);              post();
            bind(OPC.IF_CMPEQ_D);                                       iparmByte();
            bind(OPC.IF_CMPEQ_D_WIDE);            pre(FLOW_CHANGE);     do_if(2, EQ, DOUBLE);              post();
            bind(OPC.IF_CMPNE_D);                                       iparmByte();
            bind(OPC.IF_CMPNE_D_WIDE);            pre(FLOW_CHANGE);     do_if(2, NE, DOUBLE);              post();
            bind(OPC.IF_CMPLT_D);                                       iparmByte();
            bind(OPC.IF_CMPLT_D_WIDE);            pre(FLOW_CHANGE);     do_if(2, LT, DOUBLE);              post();
            bind(OPC.IF_CMPLE_D);                                       iparmByte();
            bind(OPC.IF_CMPLE_D_WIDE);            pre(FLOW_CHANGE);     do_if(2, LE, DOUBLE);              post();
            bind(OPC.IF_CMPGT_D);                                       iparmByte();
            bind(OPC.IF_CMPGT_D_WIDE);            pre(FLOW_CHANGE);     do_if(2, GT, DOUBLE);              post();
            bind(OPC.IF_CMPGE_D);                                       iparmByte();
            bind(OPC.IF_CMPGE_D_WIDE);            pre(FLOW_CHANGE);     do_if(2, GE, DOUBLE);              post();
            ...
            bind(OPC.THIS_GETFIELD_F);                                  iparmUByte();
            bind(OPC.THIS_GETFIELD_F_WIDE);       pre(FLOW_NEXT);       do_this_getfield(FLOAT);           post();
            bind(OPC.THIS_GETFIELD_D);                                  iparmUByte();
            bind(OPC.THIS_GETFIELD_D_WIDE);       pre(FLOW_NEXT);       do_this_getfield(DOUBLE);          post();
            ...
            bind(OPC.THIS_PUTFIELD_F);                                  iparmUByte();
            bind(OPC.THIS_PUTFIELD_F_WIDE);       pre(FLOW_NEXT);       do_this_putfield(FLOAT);           post();
            bind(OPC.THIS_PUTFIELD_D);                                  iparmUByte();
            bind(OPC.THIS_PUTFIELD_D_WIDE);       pre(FLOW_NEXT);       do_this_putfield(DOUBLE);          post();
